...
describe('TtsHelper', () => {
  describe('GetTtsUriFromEndpoint()', () => {
    ...
    it('throws http error', async () => {
      ...
      try {
        ...
        expect(error).toHaveProperty('Action', 'GetTtsUriFromEndpoint');
  ...
  describe('TtsOptionsToNotification()', () => {
    it('requests uri', async () => {
      ...
      expect(result).toHaveProperty('trackUri', resultUri);
    });
    ...
    (!process.env.SONOS_TTS_ENDPOINT ? it : it.skip)('throws error when endpoint not set', async () => {
      try {
        ...
        expect(error).toBeDefined();
        expect(error).toHaveProperty('message', 'TTS endpoint is required, check the documentation.');
        return;
      }
      expect(false).toBeTruthy(); // This should not be reached.
    })
    ...
    it('throws error when language not set', async () => {
      try {
        ...
        expect(error).toBeDefined();
        expect(error).toHaveProperty('message', 'TTS lang is required.');
        return;
      }
      expect(false).toBeTruthy(); // This should not be reached.
    })
    ...
    it('throws error when test not set or empty', async () => {
      try {
        ...
        expect(error).toBeDefined();
        expect(error).toHaveProperty('message', 'TTS text is required, duh!');
        return;
      }
      expect(false).toBeTruthy(); // This should not be reached.
    })

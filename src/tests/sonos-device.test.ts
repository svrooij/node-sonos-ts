...
describe('SonosDevice', () => {
  describe('LoadDeviceData()', () => {
    it('works', async () => {
      ...
      expect(device.Volume).toEqual(6);
    });
  });
  ...
  describe('ExecuteCommand()', () => {
    ...
    it('executes \'LoadUuid\'', async () => {
      ...
      expect(result).toEqual('RINCON_00FFFFFFFFBC01400');
    });
    ...
    it('executes \'ZoneGroupTopologyService.GetZoneGroupAttributes\'', async () => {
      ...
      expect(result).toHaveProperty('CurrentZoneGroupName', 'Kantoor');
      expect(result).toHaveProperty('CurrentZoneGroupID', 'fakeID');
    });
  ...
  describe('GetDeviceDescription()', () => {
    it('loads device description', async () => {
      ...
      expect(result.modelName).toEqual(playbar);
      expect(result.modelDescription).toEqual(playbar);
      expect(result.roomName).toEqual('TV');
    });
    it('throws error on http error', async () => {
      ...
      expect(false).toBeTruthy(); // Should never be reached
    });
  });
  ...
  describe('GetFavoriteRadioShows()', () => {
    it('works', async () => {
      ...
      expect(result).toHaveProperty('NumberReturned', 0);
      expect(result).toHaveProperty('TotalMatches', 0);
      expect(result).toHaveProperty('UpdateID', 1);
    });
  });
  ...
  describe('GetFavoriteRadioStations()', () => {
    it('works', async () => {
      ...
      expect(result).toBeDefined();
      expect(result).toHaveProperty('NumberReturned', 12);
      expect(result).toHaveProperty('TotalMatches', 12);
      expect(result).toHaveProperty('UpdateID', 1);
  ...
  describe('GetState()', () => {
    it('returns preloaded mute, state and volume', async () => {
      ...
      expect(device.Muted).toBeTruthy();
      expect(device.Volume).toEqual(6);
      ...
      expect(state.volume).toEqual(6);
      expect(state.transportState).toEqual(TransportState.Stopped);
  ...
  describe('GetQueue()', () => {
    it('works', async () => {
      ...
      expect(result).toBeDefined();
      expect(result).toHaveProperty('NumberReturned', 70);
      expect(result).toHaveProperty('TotalMatches', 70);
      expect(result).toHaveProperty('UpdateID', 2);
      expect(result.Result).toHaveLength(70);
    });
  });
  ...
  describe('GetZoneInfo()', () => {
    it('works', async () => {
      ...
      expect(result.MACAddress).toEqual('00:FF:FF:FF:FF:BC');
      expect(result.SerialNumber).toEqual('00-FF-FF-FF-FF-BC:A');
    });
  });
  ...
  describe('JoinGroup(...)', () => {
    ...
    it('throws error for unknown group', async () => {
      ...
      try {
        ...
        expect(error).toHaveProperty('message', `Player '${otherDevice}' isn't found!`);
        return;
      }
  ...
  describe('MusicServicesClient(...)', () => {
    it('returns Spotify client', async () => {
      ...
      expect(spotifyClient).toHaveProperty('key', randomAccountKey);
      expect(spotifyClient).toHaveProperty('authToken', randomAccountToken);
  ...
  describe('Name', () => {
    ...
    it('throws when zone attributes not loaded', () => {
      const device = new SonosDevice(TestHelpers.testHost, 1400);
      expect(() => device.Name).toThrow('Zone attributes not loaded');
    });
  });
  ...
  describe('Port', () => {
    it('returns port number', () => {
      ...
      expect(device.Port).toEqual(1500);
    });
  });
  ...
  describe('SetAVTransportURI(...)', () => {
    it('accepts x-sonosapi-stream:...', async () => {
      ...
      TestHelpers.mockRequest('/MediaRenderer/AVTransport/Control',
        '"urn:schemas-upnp-org:service:AVTransport:1#SetAVTransportURI"',
        '<u:SetAVTransportURI xmlns:u="urn:schemas-upnp-org:service:AVTransport:1"><InstanceID>0</InstanceID><CurrentURI>x-sonosapi-stream:s78122?sid=254&amp;flags=8224&amp;sn=0</CurrentURI><CurrentURIMetaData>&lt;DIDL-Lite xmlns:dc=&quot;http://purl.org/dc/elements/1.1/&quot; xmlns:upnp=&quot;urn:schemas-upnp-org:metadata-1-0/upnp/&quot; xmlns:r=&quot;urn:schemas-rinconnetworks-com:metadata-1-0/&quot; xmlns=&quot;urn:schemas-upnp-org:metadata-1-0/DIDL-Lite/&quot;&gt;&lt;item id=&quot;10092020s78122&quot; restricted=&quot;true&quot;&gt;&lt;dc:title&gt;Some radio station&lt;/dc:title&gt;&lt;upnp:class&gt;object.item.audioItem.audioBroadcast&lt;/upnp:class&gt;&lt;desc id=&quot;cdudn&quot; nameSpace=&quot;urn:schemas-rinconnetworks-com:metadata-1-0/&quot;&gt;RINCON_AssociatedZPUDN&lt;/desc&gt;&lt;/item&gt;&lt;/DIDL-Lite&gt;</CurrentURIMetaData></u:SetAVTransportURI>',
        'SetAVTransportURIResponse',
        'AVTransport',
      );
      const device = new SonosDevice(TestHelpers.testHost, 1400);
  ...
  describe('Group options in constructor', () => {
    it('returns correct groupname', (done) => {
      ...
      expect(device.GroupName).toEqual(groupName);

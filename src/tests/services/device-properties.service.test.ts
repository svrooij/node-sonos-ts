...
import { DevicePropertiesService } from '../../services/device-properties.service';
...
describe('DevicePropertiesService', () => {
  describe('GetButtonLockState', () => {
    it('works', async () => {
      ...
      expect(buttonLockState.CurrentButtonLockState).toEqual('Off');
    })
  })
  ...
  describe('GetHouseholdID', () => {
    it('works', async () => {
      ...
      expect(householdResponse.CurrentHouseholdID).toEqual('Sonos_fake_id');
    })
  })
  ...
  describe('GetLEDState', () => {
    it('works', async () => {
      ...
      expect(getLedStateResponse.CurrentLEDState).toEqual('On');
    })
  })
  ...
  describe('GetZoneAttributes', () => {
    it('works', async () => {
      ...
      expect(zoneAttributes.CurrentZoneName).toEqual('Kantoor');
      expect(zoneAttributes.CurrentIcon).toEqual('x-rincon-roomicon:office');
      expect(zoneAttributes.CurrentConfiguration).toEqual(1);
    })
  })
  ...
  describe('GetZoneInfo', () => {
    it('works', async () => {
      ...
      expect(zoneInfo.MACAddress).toEqual('00:FF:FF:FF:FF:BC');
      expect(zoneInfo.SerialNumber).toEqual('00-FF-FF-FF-FF-BC:A');
    });
  });
  ...
  describe('Event parsing', () => {
    it('works', (done) => {
      ...
      service.Events.once('serviceEvent', (data) => {
        expect(data.Icon).toEqual('x-rincon-roomicon:office');
        expect(data.SupportsAudioClip).toBe(false);
        expect(data.SupportsAudioIn).toBeTruthy();
        expect(data.WifiEnabled).toBe(false);
        expect(data.WirelessLeafOnly).toBe(false);
        expect(data.WirelessMode).toEqual(0);
        expect(data.ZoneName).toEqual('Kantoor');

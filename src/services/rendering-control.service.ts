...
export class RenderingControlServiceBase extends BaseService<RenderingControlServiceEvent> {
   ...
   *
   * @param {number} input.InstanceID - InstanceID should always be `0`
   * @param {string} input.EQType - Allowed values `DialogLevel` (bool) / `MusicSurroundLevel` (-15/+15) /  `NightMode` (bool) / `SubGain` (-10/+10) / `SurroundEnable` (bool) / `SurroundLevel` (-15/+15) / `SurroundMode` (0 = full, 1 = ambient) / `HeightChannelLevel` (-10/+10)
